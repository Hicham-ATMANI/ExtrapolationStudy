...
void ExtractHighmu5Intervals(){
          ...
          for( int i=0 ; i < Nbins  ; i++){ 
ExtraCorr->SetBinContent(i+1,Extra->GetBinContent(i+1));
//ExtraCorr->SetBinContent(i+1,Extra->GetBinContent(i+1)+HistTCStudy->GetBinContent(i+1));
...
void ExtractHighmu4Intervals(){
          ...
          Double_t y[4] = {22.47,31.05,39.63,53.67};
          ...
          TFile* Fichier = new TFile("output-24-4Intervals.root","recreate");
          MAT->Write("colz"); Chi2->Write();
          for( int i=0 ; i < Nbins  ; i++){
          vectGraph[i]->Write();}
          Extra->Write();
